...
public:
        ...
        m_shared_memory(boost::interprocess::open_or_create, shared_memory_name.c_str(), 1024)
    {
        ...
        m_users = m_shared_memory.find_or_construct<counter_t>("m_users")();
        m_messages = m_shared_memory.find_or_construct<counter_t>("m_messages")();
...
private:
    ...
    void write()
    {
        for (std::string str; std::getline(std::cin, str); send_message(m_user_name + ':' + '\t' + str));
    }
